...
namespace gamingCloud
{
    ...
    public class GameUtilities : HttpRequest
    {
        ...
        /// <summary>
        /// use this method to get Server Time.
        /// </summary>
        public static async Task<DateTime?> GetServerDateTime()
        {
            ServerResponse req = await HttpRequest.GetRequestAsync("/game/serverTs");
            if(req.IsSuccess == true)
            {
                double ts = JObject.Parse(req.responseMessage)["ts"].ToObject<double>();
                return ConvertToDateTime(ts);
            }
            return null;
        ...
        public static async Task<ApiResponseArray> GetTableDataByName(string TableName)
        {
            ...
            JArray resp = JArray.Parse(req.responseMessage);
            if (req.IsSuccess)
            {
        ...
        public static async Task<ApiResponseArray> GetTableDataByName(string TableName, Dictionary<string, string> query)
        {
            string queryParam = GenerateQueryParameter(query);
            ServerResponse req = await HttpRequest.GetRequestAsync("/DBaaS/byName/" + TableName + queryParam);
        ...
        public static async Task<ApiResponseArray> GetTableDataById(string TableId, Dictionary<string, string> query)
        {
            string queryParam = GenerateQueryParameter(query);

            ServerResponse req = await HttpRequest.GetRequestAsync("/DBaaS/byId/" + TableId + queryParam);
        ...
        private static string GenerateQueryParameter(Dictionary<string, string> query)
        {
            string result = "?";
            int i = 0;
