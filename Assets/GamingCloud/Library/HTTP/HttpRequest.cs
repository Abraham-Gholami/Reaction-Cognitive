...
namespace gamingCloud
{
    ...
    public class HttpRequest : GCPolicy
    {
        ...
        //static string baseURL = "http://172.16.73.126:4000";
        ...
        async static public Task<ServerResponse> PostRequestAsync(string route, JObject body)
        {
            HttpClient client = await Client();
        ...
        async static public Task<ServerResponse> PutRequestAsync(string route, JObject body)
        {
            HttpClient client = await Client();
            //var bodyContent = new FormUrlEncodedContent(body);
            var bodyContent = new StringContent(body.ToString(Formatting.None), Encoding.UTF8, "application/json");
